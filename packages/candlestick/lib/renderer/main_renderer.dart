...
//    }
  ...
  void drawSAR(CandleEntity lastPoint, CandleEntity curPoint, Canvas canvas,
    ...
    if (sar == halfHL) {
      ...
      color = this.chartColors.upColor; // SAR ：（/）
    } else {
      color = this.chartColors.dnColor; // SAR ：（/）
    }
    final lineWidth = _calculateMainIndicatorWidth();
    final radius = lineWidth.clamp(1.5, 3.0); //
  ...
  void drawCandle(CandleEntity curPoint, Canvas canvas, double curX) {
    var high = getY(curPoint.high); //  Y
    var low = getY(curPoint.low); //  Y
    var open = getY(curPoint.open); //  Y
    var close = getY(curPoint.close); //  Y
